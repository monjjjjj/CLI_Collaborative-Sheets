...
/*
 *Visitor
 *declares a Visit operation for each class of ConcreteJob
...
class Database
{
    ...
    vector<vector<float>> get_sheet(string user_name, string sheet_name)
    {
        ...
        if (!check_readable(user_name, sheet_name)) throw invalid_argument( "User does not have permission to read!!!\n" );
    ...
    vector<vector<float>> update_sheet(string user_name, string sheet_name, int x, int y, float new_value)
    {
        ...
        if (!check_editable(user_name, sheet_name)) throw invalid_argument( "User does not have permission to edit!!!\n" );
    ...
    private:
        ...
        void print_user_map()
        {
            for (const auto &s: user_map)
            {
                cout << "user id: " << s.first << ", name: " << s.second << "\n";
            }

            cout << "\n";
        }

        void print_sheet_editable_map()
        {
            for (const auto &sheet_map: sheet_permission_map)
            {
                cout << "sheet id: " << sheet_map.first << ", can edit by user: " << endl;
                for (const auto &permission_map: sheet_map.second)
                {
                    if (permission_map.second)
                    {
                        cout << "\"" << permission_map.first << "\", ";
                    }
                }

                cout << "can read by user: ";
                for (const auto &permission_map: sheet_map.second)
                {
                    if (!permission_map.second)
                    {
        ...
        void print_sheet_data_map()
        {
            for (const auto &sheet_map: sheet_data_map)
            {
                cout << "sheet id: " << sheet_map.first << ", data: " << endl;
                for ( const auto &row : sheet_map.second )
                {
